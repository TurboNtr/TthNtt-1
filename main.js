...
async function starts() {
  ...
  Turbo.on('group-participants-update', async (anu) => {
      ...
      if (anu.action == 'add') {
        ...
        Turbo.sendButtonLoc(mdata.id, buff, capt, 'Apoya al Bot en YouTube\nhttps://youtube.com/c/turbontr1?sub_confirmatión=1', '👋🏻 🐈💨', 'unde', false, {
	      contextInfo: { 
            mentionedJid: Turbo.parseMention(capt)
	      } 
	    });
...
Turbo.on('group-participants-update', async (anu) => {
  metdata = await Turbo.groupMetadata(anu.jid);
  ...
  try {
	      ...
	      ppimg = 'https://i.ibb.co/F6xP0mp/Profile-turbo.jpg';
	    } 
...
╽ ⋆✯✮NUEVO ADMIN✮✯⋆
├❥⃔⃕≻ Nombre: ${username}
├❥⃔⃕≻ Numero: @${num.split('@')[0]}
├❥⃔⃕≻ Info* : ${about} 
├❥⃔⃕≻ Hora: ${time}
╰•⋆҈͜͡ ⫘𝐓𝐔𝐑𝐁𝐎⋆𝐍𝐈𝐓𝐑𝐎⫘⋆҈͜͡•╯`;
    return Turbo.sendMessage(metdata.id, img, MessageType.image, {caption: capt, contextInfo: {'mentionedJid': [num]}});
...
├❥⃔⃕≻ Hora : ${time}
╰•⋆҈͜͡ ⫘𝐓𝐔𝐑𝐁𝐎⋆𝐍𝐈𝐓𝐑𝐎⫘⋆҈͜͡•╯`;
    Turbo.sendMessage(metdata.id, img, MessageType.image, {caption: capt, contextInfo: {'mentionedJid': [num]}});
  }
})
...
╽✫✫✫𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀✫✫✫
╽
╽᳆͜͡✪ Nombre : @${m.participant.split`@`[0]} 
╽᳆͜͡✪ Hora : ${time}
╽
╽         𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀
╰─✦─⋆❮✦⋆.☬.⋆✦❯⋆─✦─╯
